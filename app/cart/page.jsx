...
const Page = () => {
    ...
    return (
        <div className="flex flex-col py-16 md:py-5 min-h-screen">
            ...
            <Link href="/products"><p className="text-right mt-10 mb-2 text-sm font-semibold underline text-customDark2">Continue shopping</p></Link>
            {totalQuantity > 0 ? (
                <>
                <div className={`overflow-x-auto rounded-md mb-10`}>
                    <table className="min-w-full text-left border-spacing-y-1">
                        ...
                        <tbody className={`block ${isOpen ? "md:block lg:table-row-group" : "md:table-row-group"} w-full`}>
                            {cart.map((item) => (
                                <tr key={item.id} className={`bg-white border-b-2 border-t-2 border-[#eeeeee] block ${isOpen ? "md:block lg:table-row" : "md:table-row"}`}>
                ...
                <div className="flex justify-center">
                    <motion.button 
                        className="bg-red-700 text-white font-base font-semibold px-5 py-2 w-fit rounded-md"
