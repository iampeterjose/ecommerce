...
const NewArrivals = () => {
    const { newArrivals, isOpen } = useProductStore();
    ...
    return (
        ...
        <div className="border-t border-customBlue py-2 w-full">
            <div className="flex justify-between items-center">
                ...
                <Link href="/products/newarrivals"><span className="text-xs md:text-sm font-semibold text-blue-500 cursor-pointer">Show all</span></Link>
            </div>
            <ul className={`grid lg:grid-cols-5 md:grid-cols-${isOpen ? '2' :'3'} grid-cols-2 gap-3 lg:gap-2`}>
            {newArrivals.length > 0 ? newArrivals.slice(0,5).map((product) => (
