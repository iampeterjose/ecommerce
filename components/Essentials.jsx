...
const Essentials = () => {
    const { essentials, isOpen } = useProductStore();
    ...
    return (
        <div className="border-t border-customBlue py-2 w-full">
            <div className="flex justify-between items-center">
                ...
                <Link href="/products/essentials"><span className="text-xs md:text-sm font-semibold text-blue-500 cursor-pointer">Show all</span></Link>
            </div>
            <ul className={`grid lg:grid-cols-5 md:grid-cols-${isOpen ? '2' :'3'} grid-cols-2 gap-3 lg:gap-2`}>
            {essentials.length > 0 ? essentials.slice(0,5).map((product) => (
                ...
                <p className="text-slate-700 font-semibold">No essentials products yet.</p>
            )}
            </ul>
        </div>
    )
